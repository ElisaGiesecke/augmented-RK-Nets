...
def run_experiment(data, network, training, reps=10, save_plots=True, 
    ...
    results['statistics'] = {'epochs': [], 'cost_train': [], 'acc_train': [], 'cost_val': [], 
           'acc_val':[]}
    ...
    for i in range(reps):
       ...
       # image data
        elif data['data_example'] == 'MNIST':
            training_data = datasets.MNIST(root="image_data", train=True, download=True,
                                           transform=Compose([ToTensor(), Lambda(lambda t: torch.flatten(t))]),
                                           target_transform = Lambda(lambda y: torch.zeros(10, dtype=torch.float).scatter_(dim=0, index=torch.tensor(y), value=1)))
            test_data = datasets.MNIST(root="image_data", train=False, download=True,
                                       transform=Compose([ToTensor(), Lambda(lambda t: torch.flatten(t))]),
                                       target_transform = Lambda(lambda y: torch.zeros(10, dtype=torch.float).scatter_(dim=0, index=torch.tensor(y), value=1)))

            train_dataloader = DataLoader(training_data, batch_size=data['N'], shuffle=True)
            test_dataloader = DataLoader(test_data, batch_size=data['V'], shuffle=True)
        ...
        elif data['data_example'] == 'FashionMNIST':   
            training_data = datasets.FashionMNIST(root="image_data", train=True, download=True,
                                                   transform=Compose([ToTensor(), Lambda(lambda t: torch.flatten(t))]),
                                                   target_transform = Lambda(lambda y: torch.zeros(10, dtype=torch.float).scatter_(dim=0, index=torch.tensor(y), value=1)))
            test_data = datasets.FashionMNIST(root="image_data", train=False, download=True,
                                               transform=Compose([ToTensor(), Lambda(lambda t: torch.flatten(t))]),
                                               target_transform = Lambda(lambda y: torch.zeros(10, dtype=torch.float).scatter_(dim=0, index=torch.tensor(y), value=1)))
        ...
        results['statistics']['epochs'].append(len(cost_train)-1)
